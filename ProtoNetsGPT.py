...
with open("prototypes.json", "r", encoding="utf-8") as file:
...
def parcourir_items(list_items, a):
    ...
    for label in range(5):
        print(f"Label {label} trouvés : {résultats_distance[label]} corrects : {résultats_corrects_label_distance.get(label, 0)} attendus : 500")

    print(f"Nombre de résultats corrects avec le calcul de similarité: {résultats_corrects_similarity}")
    for label in range(5):
        print(f"Label {label} trouvés : {résultats_similarity[label]} corrects : {résultats_corrects_label_similarity.get(label, 0)} attendus : 500")
